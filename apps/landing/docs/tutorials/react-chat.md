...
This walks the essentials of the reference app ([apps/chat](https://github.com/in-th3-l00p/hermes-remote/tree/main/apps/chat)): a chat with streaming, markdown, attachments, reactions, and edits.
...
`--anonymous` keeps this tutorial friction free; switch to real auth with [the Supabase tutorial](/tutorials/supabase-auth).
...
Use `useSessions` for the list and `chat.open(id)` / `chat.reset()` for switching, as shown in [the React hooks page](/clients/react). Persist ids in localStorage when running anonymously.
