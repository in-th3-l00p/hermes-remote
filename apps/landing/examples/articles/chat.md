...
`useChat` and `useSessions` from `@in-th3-l00p/hermes-remote-react`.

## The client

The app creates a single anonymous `HermesClient` pointed at the sandbox
backend. In production the base URL is simply `/api/hermes` — the examples and
the server share an origin, so there is no CORS configuration anywhere:

```ts
import { HermesClient } from "@in-th3-l00p/hermes-remote-client";

export const client = new HermesClient({ baseUrl: "/api/hermes" });
```
