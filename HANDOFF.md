...
* **GitHub Packages:** all three packages published under scope `@in-th3-l00p` to `npm.pkg.github.com` at 1.0.0 (a stray 0.2.0 also exists from a failed `&&` chain — harmless). Release workflow publishes automatically on `v*` tags using `GITHUB_TOKEN`.
* **Release process:** bump versions in the three package.json files → commit → `git tag v1.x.y` → push tag → `release.yml` tests, publishes, packs tarballs, creates/updates the GitHub release.
